...
class SequencesController extends Controller {
    ...
    public function actionSearch_seq_treatment() {
        ...
        if ($_POST['form'] == 'wellFormedFormula') {
            $functionArray[0] = $_POST['Sequences']['wellFormedFormula'];
        }
        if ($_POST['form'] == 'BinaryNumber') {
            $functionArray[0] = $_POST['Sequences']['BinaryNumber'];
        }
        if ($_POST['form'] == 'MultipleFunction') {
            $functionArray = explode("\n", $_POST['Sequences']['MultipleFunction']);
        }
        ...
        foreach ($functionArray as $key => $value) {
            try {
                ...
                $searchResult = Sequences::find()->where('ndf > :ndf', [':ndf' => $dnfArray[0]])->orderBy('weak_constraint')->all();
